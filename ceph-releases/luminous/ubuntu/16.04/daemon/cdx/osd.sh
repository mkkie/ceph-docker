...
function activate_osd {
  ...
  "$DOCKER_CMD" run -d -l CLUSTER="${CLUSTER}" -l CEPH=osd -l DEV_NAME="${disk2act}" -l OSD_ID="${OSD_ID}" \
    ...
    -e CDX_ENV="${CDX_ENV}" -e OSD_BLUESTORE="${OSD_BLUESTORE}" -e DEBUG="${DEBUG}" -e OSD_DEVICE="${disk2act}" \
...
function prepare_new_osd {
  ...
  if "$DOCKER_CMD" run -l CLUSTER="${CLUSTER}" -l CEPH=osd_prepare -l DEV_NAME="${osd2prep}" --name="${CONT_NAME}" \
    --privileged=true -v /dev/:/dev/ -e CDX_ENV="${CDX_ENV}" -e OSD_BLUESTORE="${OSD_BLUESTORE}" \
    -e OSD_DEVICE="${osd2prep}" "${DAEMON_VERSION}" osd_ceph_disk_prepare &>/dev/null; then
    return 0
  else
    return 1
  fi
